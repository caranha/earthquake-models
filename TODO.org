...
- [X] Reading earthquakes from files, filtering them by time and
  space.
- [ ] Define an interface for N-dimensional models 
  (So I can define the definitions by parameter file)
- [ ] 2-dimensional model (latitude, longitude)
- [ ] Assign events to model from data
- [ ] Generate random model (from Total number of earthquakes)
- [ ] Calculate log likelyhood of random model against data model
- [ ] Random Search
