...
- [ ] Prediction model: Defines bins and number of predicted
  earthquake in each bin
- [ ] Models must report their defining qualities (spatial, temporal,
  bin dimensions, etc)
- [ ] Models must report loglikelyhood when compared with a set of
  data (ignoring extraneous data)
- [ ] Implement Multi-dimensional Models
